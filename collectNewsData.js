...
function getNewsRestApiUrl(withoutCache = true) {
  let newsRestApiUrl =
    "https://actu.epfl.ch/api/v1/channels/1/news/?format=json&limit=1&lang=en";
  if (withoutCache) {
...
async function getLastFromAPI() {
  ...
  lastNews["x-varnish-cache"] = response.headers["x-varnish-cache"] || 'no-information';
  lastNews["cache-control"] = response.headers["cache-control"] || 'no-information';
...
async function scrapeNewsFrom(url) {
  ...
  news["cf-cache-status"] = response.headers["cf-cache-status"] || 'no-information' ;
  news["x-varnish-cache"] = response.headers["x-varnish-cache"] || 'no-information';
  news["cache-control"] = response.headers["cache-control"] || 'no-information';
...
async function main(cache) {
  ...
  let urlWithVarnishAndCloudflare = "https://www.epfl.ch/en";
  ...
  if (!cache.hasOwnProperty(key1)) {
    ...
    msDiff = Math.abs(new Date(newsFromWww["call_api_date"]) - new Date(publishedDateFromApi));
  ...
  if (!cache.hasOwnProperty(key2)) {
    ...
    msDiff = Math.abs(new Date(newsFromWithOutCache["call_api_date"]) - new Date(publishedDateFromApi));
  ...
  let url3 =
    "https://www.epfl.ch/campus/services/website/canari/actu-without-varnish/";
  ...
  if (!cache.hasOwnProperty(key3)) {
    ...
    msDiff = Math.abs(new Date(newsFromWithOutVarnish["call_api_date"]) - new Date(publishedDateFromApi));
  ...
  if (!cache.hasOwnProperty(key4)) {
    ...
    msDiff = Math.abs(new Date(newsFromWithOutCloudflare["call_api_date"]) - new Date(publishedDateFromApi));
